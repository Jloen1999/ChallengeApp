...
public class RetoController {
	...
	public String reiniciarReto(@RequestParam("retoId") Integer retoId, HttpSession session) {
		...
		progresoRetoService.reiniciarReto(usuario,reto);

		return "redirect:/usuario/reto/"+retoId;
	}
	
	@PostMapping("/desunirse")
	public String desunirseReto(@RequestParam("retoId") Integer retoId, HttpSession session) {
		Usuario usuario = (Usuario) session.getAttribute("userActual");
		if (usuario == null) {
			return "redirect:/login";
		}
		...
		Reto reto=retoService.obtenerReto(Long.valueOf(retoId));
		participantesRetoService.desunirseReto(usuario,reto);
		progresoRetoService.borrarProgresoReto(usuario,reto);
	...
	private void asignarPuntosPorReto(Usuario usuario, Reto reto) {
		...
		punto.setCantidad(reto.getTipo() == Tipo.SIMPLE ? (tiempoCompletado < 24 ? 100 : 10)
	...
	private void gestionarRecompensas(Usuario usuario, List<Reto> retosCompletados) {
		...
		gestionarMedallas(usuario, TipoMedalla.PLATA,
				recompensaService.obtenerRecompensasBronceUsuario(usuario).size() / 5);
		gestionarMedallas(usuario, TipoMedalla.ORO,
				recompensaService.obtenerRecompensasPlataUsuario(usuario).size() / 5);
